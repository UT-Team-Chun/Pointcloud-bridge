...
from utils.BridgeDataLoader import LWBridgeDataset
...
def train():
    ...
    # 创建数据加载器
    root = Path('data/fukushima/org')

    print("start loading training data ...")
    train_dataset = LWBridgeDataset(split='train', data_root=root, num_point=config['num_points'], block_size=1.0, sample_rate=1.0, num_class=config['num_classes'], transform=None)
    print("start loading test data ...")
    val_dataset = LWBridgeDataset(split='val', data_root=root, num_point=config['num_points'], block_size=1.0, sample_rate=1.0, num_class=config['num_classes'], transform=None)

    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True, num_workers=0, pin_memory=True, drop_last=True)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=config['batch_size'], shuffle=False, num_workers=0, pin_memory=True, drop_last=True)
