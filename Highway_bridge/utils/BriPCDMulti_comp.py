...
class BriPCDMulti(Dataset):
    ...
    def _preprocess_files(self):
        ...
        for filename in pbar:
            ...
            points, labels = self.preprocessor.process_point_cloud(points, labels)
...
class BridgePointCloudProcessor:
    ...
    def voxel_downsample(self, points, rgb, labels):
        ...
        # 将RGB和label作为特征
        features = np.column_stack([rgb, labels])
        ...
        down_features = np.asarray(downsampled.colors)

        down_rgb = down_features[:, :3]
        down_labels = down_features[:, 3].astype(int)
