...
def train(config=None):
    ...
    model = RandLANet(num_classes).to(device)
    ...
    for epoch in range(1, config['num_epochs']+1):
        ...
        for batch in pbar:
            ...
            labels = batch['labels'].to(device) # [B, N] - 保持原始逐点标签

            optimizer.zero_grad()
            # 移除 return_global=True
            outputs = model(points, colors) # 模型现在输出 [B, C, N]

            # CrossEntropyLoss 输入: [B, C, N], 目标: [B, N]
            loss = criterion(outputs, labels) # 直接使用逐点标签

            loss.backward()
            optimizer.step()

            # 计算准确率 (逐点比较)
            # outputs: [B, C, N] -> pred: [B, N]
            pred = outputs.argmax(dim=1)
            acc = pred.eq(labels).float().mean() # 逐点比较
        ...
        with torch.no_grad():
            ...
            for batch in pbar:
                ...
                labels = batch['labels'].to(device, non_blocking=True) # [B, N] - 保持原始逐点标签

                # 移除 return_global=True
                outputs = model(points, colors) # 模型输出 [B, C, N]
                ...
                # outputs: [B, C, N] -> pred: [B, N]
                pred = outputs.argmax(dim=1)
                acc_1 = pred.eq(labels).float().mean() # 逐点比较
                ...
                all_labels.append(labels) # 存储逐点标签
        ...
        all_preds = torch.cat(all_preds, dim=0) # [TotalPointsInValSet]
        all_labels = torch.cat(all_labels, dim=0) # [TotalPointsInValSet]
