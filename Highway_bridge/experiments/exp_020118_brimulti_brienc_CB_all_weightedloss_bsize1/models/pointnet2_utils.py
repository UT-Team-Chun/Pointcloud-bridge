...
class PointTransformerFusion(nn.Module):
    def __init__(self, feature_dims=[64, 128, 256, 512], unified_dim=128, output_dim=960, num_heads=8, num_layers=2, dropout=0.1):
        super(PointTransformerFusion, self).__init__()

        # 统一特征维度的转换层
        self.unified_dim = unified_dim  # 统一的特征维度
        ...
        self.residual_conv = nn.ModuleList([
            nn.Conv1d(dim, output_dim, 1) for dim in feature_dims
        ])
    ...
    def forward(self, l0_features, l1_features, l2_features, l3_features):
        ...
        for i, (feat, conv, res_conv) in enumerate([
            (l0_features, self.feature_conv[0], self.residual_conv[0]),
            (l1_features, self.feature_conv[1], self.residual_conv[1]),
            (l2_features, self.feature_conv[2], self.residual_conv[2]),
            (l3_features, self.feature_conv[3], self.residual_conv[3])
        ]):
            f = conv(feat)
            r = res_conv(feat)
            if i > 0:
                f = torch.nn.functional.interpolate(
                    f, size=target_points, mode='nearest')
                r = torch.nn.functional.interpolate(
                    r, size=target_points, mode='nearest')
            features_list.append(f)
            residuals.append(r)

        # 特征堆叠和变形
        features = torch.stack([f.transpose(1, 2) for f in features_list], dim=2)
        ...
        output = output + sum(residuals)    # 残差连接
