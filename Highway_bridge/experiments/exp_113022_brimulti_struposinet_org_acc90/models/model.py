...
class EnhancedPointNet2(nn.Module):
    def __init__(self, num_classes=8):
        ...
        self.sa1 = MultiScaleSetAbstraction(1024, [0.1, 0.2],[16, 32], in_chanel, [64, 128, 128])
        # 2nd layer: input = 128*2 (Multi-scale connection)
        self.sa2 = MultiScaleSetAbstraction(256,[0.2, 0.4],[16, 32], 259,[128, 192, 256])
        self.sa3 = MultiScaleSetAbstraction(64,[0.4, 0.8],[16, 32], 515,[256, 384, 512])
        #  npoint=1024,radius_list=[0.1, 0.2], nsample_list=[16, 32], in_channel=6+64, mlp=[64, 64, 128]

        # attention module
        self.attention1 = EnhancedAttentionModule(128) #128 * 2
        self.attention2 = EnhancedAttentionModule(256)
        self.attention3 = EnhancedAttentionModule(512)
        ...
        # Decoder
        self.fp3 = FeaturePropagation(1536, [256, 256])  # multi: 512*2 + 256*2 ,1536 ; 256 + 256*2 ,768
    ...
    def forward(self, xyz, colors):
        ...
        points = torch.cat([pos_enc, colors], dim=1)  # Merge Features: [B, 67, N]
