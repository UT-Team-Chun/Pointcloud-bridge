...
class SegmentationHead(nn.Module):
    def __init__(self, in_channels, num_classes, dropout_prob=0.3):
        ...
        self.head = nn.Sequential(
            nn.Conv1d(in_channels, in_channels // 2, 1),  # 将输入特征降维
            nn.BatchNorm1d(in_channels // 2),  #
            nn.ReLU(),  # 激活函数
            nn.Dropout(p=dropout_prob),  # Dropout
            nn.Conv1d(in_channels // 2, num_classes,1)  # 映射到类别
        )
...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        if not self.cls_mode:
            self.seg_head = SegmentationHead(dec_channels[0], num_classes) #dec_channels[0]
    ...
    def forward(self, data_dict):
        ...
        #     in_channels = point["feat"].shape[1]  # 动态获取特征维度
        ...
        if not self.cls_mode:
            ...
            # 提取特征、批次索引和偏移量
            feat = point['feat']  # 点云特征
            batch = point['batch']  # 每个点的批次索引
            offset = point['offset']  # 每个 batch 的起始点索引

            # 根据 batch 分组特征
            batch_size = offset.size(0)  # batch 的数量
            features = []  # 存储每个 batch 的特征
            for i in range(batch_size):
                start = 0 if i == 0 else offset[i - 1].item()
                end = offset[i].item()
                features.append(feat[start:end])  # 提取每个 batch 的特征

            # 将每个 batch 的特征堆叠成 [B, N, C]
            features = torch.nn.utils.rnn.pad_sequence(features, batch_first=True)  # [B, N, C]
            features = features.permute(0, 2, 1)  # 调整为 [B, C, N]

            # 前向传播
            logits = self.seg_head(features)
            return logits  # 返回每个点的类别概率分布
