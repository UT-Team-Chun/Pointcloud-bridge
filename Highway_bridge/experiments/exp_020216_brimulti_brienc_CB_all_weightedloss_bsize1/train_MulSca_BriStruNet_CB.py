...
from models.model import EnhancedPointNet2, BridgeStructureLoss
...
def train():
    ...
    class_weights = torch.tensor([0.50, 2.60, 1.0, 1.46, 3.0])
    ...
    for epoch in range(1, config['num_epochs']+1):
        ...
        for batch in pbar:
            ...
            colors = batch['colors'].to(device) # [B, N, C]
            ...
            # 注意这里传入合并后的坐标信息
            loss = criterion(outputs, labels, points)
            loss.backward()
            # 梯度裁剪（按范数）
