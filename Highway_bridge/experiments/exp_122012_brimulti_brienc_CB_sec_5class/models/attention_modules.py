...
class BridgeStructureEncoding(nn.Module):
    ...
    def forward(self, xyz):
        B, N, C = xyz.shape  # B是batch size, N是每个点云中的点数, C是坐标维度
        device = xyz.device

        # 1. 粗略K近邻搜索
        dist = torch.cdist(xyz, xyz)  # [B, N, N]
        k_coarse = min(self.k * 4, N)  # 粗略搜索更多的邻居
        _, idx_coarse = dist.topk(k_coarse, dim=-1, largest=False)  # [B, N, k_coarse]

        # 2. 在粗略邻居中进行精细K近邻搜索
        idx_base = torch.arange(0, B, device=device).view(-1, 1, 1) * N
        idx_coarse = idx_coarse + idx_base  # [B, N, k_coarse]

        # 重塑索引以获取邻居
        neighbors_coarse = xyz.view(B * N, -1)[idx_coarse.view(-1)].view(B, N, k_coarse, -1)  # [B, N, k_coarse, C]

        # 计算精细距离
        k_fine = min(self.k, k_coarse)  # 精细搜索较少的邻居
        ...
        for b in range(B):
            for n in range(N):
                # 获取当前点的粗略邻居
                curr_neighbors = neighbors_coarse[b, n]  # [k_coarse, C]
                # 计算这些邻居之间的距离
                curr_dist = torch.cdist(curr_neighbors, curr_neighbors)  # [k_coarse, k_coarse]
                # 选择最近的k_fine个邻居
                _, curr_idx = curr_dist.topk(k_fine, dim=-1, largest=False)  # [k_coarse, k_fine]
                # 获取这些邻居的坐标
                curr_fine_neighbors = curr_neighbors[curr_idx[0]]  # [k_fine, C]
                neighbors_final.append(curr_fine_neighbors)

        # 重塑结果为所需的形状
        neighbors = torch.stack(neighbors_final).view(B, N, k_fine, -1)  # [B, N, k_fine, C]

        # 扩展中心点
        center = xyz.unsqueeze(2).expand(-1, -1, k_fine, -1)  # [B, N, k_fine, C]

        # 计算相对位置
        rel_pos = neighbors - center  # [B, N, k_fine, C]
        ...
        structure_features = structure_features.unsqueeze(2).expand(-1, -1, self.k, -1)  # [B, N, k, 13]
