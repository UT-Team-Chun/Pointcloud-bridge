...
#from .pointnet2_utils import three_interpolate, three_nn # 假设这些函数已在pointnet2_utils中定义或可以从其他地方导入
...
class RandLANet(nn.Module):
    def __init__(self, num_classes=5, d_in=3):
        ...
        # 它应该包含编码器各层（包括初始层）的输出通道数，用于跳跃连接
        # encoder_features 列表包含 [initial(8), enc0(16), enc1(64), enc2(128), enc3(256)]
        # 解码器 i=0 需要 enc2(128)
        # 解码器 i=1 需要 enc1(64)
        # 解码器 i=2 需要 enc0(16)
        # 解码器 i=3 需要 initial(8)
    ...
    def forward(self, xyz, features=None): # 移除 return_global 参数
        ...
        for i in range(len(self.up_modules)):
            ...
            # encoder_features 列表包含 [initial(8), enc0(16), enc1(64), enc2(128), enc3(256)]
            # encoder_xyz 列表包含 [orig_xyz, xyz0, xyz1, xyz2, xyz3]
            # i=0: xyz_skip=xyz2, features_skip=enc2(128), xyz_prev=xyz3
            # i=1: xyz_skip=xyz1, features_skip=enc1(64), xyz_prev=xyz2
            # i=2: xyz_skip=xyz0, features_skip=enc0(16), xyz_prev=xyz1
            # i=3: xyz_skip=orig_xyz, features_skip=initial(8), xyz_prev=xyz0
