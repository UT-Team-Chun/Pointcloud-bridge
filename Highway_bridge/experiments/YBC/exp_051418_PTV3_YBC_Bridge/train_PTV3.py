...
def train(config=None):
    if config is None:
        config = {
            ...
            'case' : 'PTV3_YBC_Bridge',
            'train_dir': 'data/YBC/train',
            'val_dir': 'data/YBC/val',
        }

    # 创建实验目录
    timestamp = datetime.datetime.now().strftime('%m%d%H')
    case = config['case']
    exp_dir = Path(f'experiments/YBC/exp_{timestamp}_{case}')
    ...
    # # wandb初始化
    # wandb.init(
    #     project="bridge-segmentation-ubuntu",  # 你的项目名称
    ...
    model=PointTransformerV3(num_classes, d_in=in_channels,embed_dim=384, # 嵌入维度
                                    depth=8,  # 深度
                                    num_heads=2, # 多头注意力机制
                                    use_flash=True # 启用Flash Attention加速
                                    ).to(device)
    ...
    shutil.copy2(train_file, exp_dir/train_file)
