...
class BriPCDMulti(Dataset):
    def __init__(self, data_dir, file_list=None, num_points=4096, transform=False, block_size=1.0,
        ...
        # 定义缓存文件夹
        self.cache_dir = os.path.join(self.data_dir, 'cache_global')
    ...
    def _assign_points_to_blocks(self, points, colors, labels, filename):
        ...
        # 计算原始类别分布比例
        original_proportions = {cls: count / total_points for cls, count in class_counts.items()}
        
        # 计算平均比例
        avg_proportion = 1.0 / len(unique_classes)
        
        # 为稀有类别增加权重，常见类别略微减少
        boost_factor = 1.3  # 稀有类别提升因子
        desired_class_proportions = {}
        for cls in unique_classes:
            if original_proportions[cls] < avg_proportion:
                # 对于稀有类别，适度增加其比例
                desired_class_proportions[cls] = min(original_proportions[cls] * boost_factor, avg_proportion * 1.5)
            else:
                # 对于常见类别，保持或略微减少
                desired_class_proportions[cls] = original_proportions[cls] * 0.9
        
        # 规范化比例，确保总和为1
        total_proportion = sum(desired_class_proportions.values())
        desired_class_proportions = {cls: prop / total_proportion for cls, prop in desired_class_proportions.items()}
        
        # 记录日志
        self.logger.info(f"Original class distribution: {original_proportions}")
        self.logger.info(f"Adjusted sampling distribution: {desired_class_proportions}")
