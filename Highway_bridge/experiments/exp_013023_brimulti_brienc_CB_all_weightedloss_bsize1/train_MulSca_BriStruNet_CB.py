...
from models.model import EnhancedPointNet2, StructureOrientedLoss
# from models.model import PointNet2
from utils.BriPCDMulti_new import BridgeDataset
...
def train():
    ...
    case = 'brimulti_brienc_CB_all_weightedloss_bsize1'
    exp_dir = Path(f'experiments/exp_{timestamp}_{case}')
    exp_dir.mkdir(parents=True, exist_ok=True)

    # 可視化するためのtensorboardとloggerを初期化する # "tensorboard --logdir"これで実行する
    writer = SummaryWriter(exp_dir / 'tensorboard')
    logger = initialize_logger(exp_dir)
    wandb.init(project="bridge", name=f"exp_{timestamp}_{case}")
    ...
    model = EnhancedPointNet2(num_classes).to(device) #EnhancedPointNet2を使う
    #model = PointNet2(num_classes).to(device) # PointNet2を使う
    ...
    optimizer = optim.Adam(model.parameters(), lr=config['learning_rate'], betas=(0.9, 0.999), weight_decay=0.01)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        mode='max',  # モニタリングする指標の種類
        factor=0.1,  # 学習率を減らす割合
        patience=5  # 指定したエポック数の間改善がない場合に学習率を減らす
    )
    ...
    criterion = StructureOrientedLoss(num_classes=5).to(device)
    ...
    for epoch in range(1, config['num_epochs']+1):
        ...
        for batch in pbar:
            ...
            colors = batch['features'].to(device)
            labels = batch['labels'].to(device)

            # 合并坐标和颜色特征
            combined_points = torch.cat([points, colors], dim=-1)  # [B, N, 3+C]
        ...
        with torch.no_grad():
            ...
            for batch in pbar:
                ...
                colors = batch['features'].to(device, non_blocking=True)
