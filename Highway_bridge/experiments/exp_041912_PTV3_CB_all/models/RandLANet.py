...
class RandLANet(nn.Module):
    def __init__(self, num_classes=5, d_in=3):
        ...
        # 原始论文中的采样率是逐渐减小的
        self.sampling_ratios = [0.35, 0.25, 0.25, 0.25]
    ...
    def forward(self, xyz, features=None):
        ...
        输入:
            xyz: 点云坐标 [B, N, 3]
            features: 点云特征 [B, N, d_in-3] 或 None
        输出:
            point_scores: 每个点的类别得分 [B, num_classes, N] 或 [B, N, num_classes]
        """
        batch_size, num_points, _ = xyz.shape
        
        # 处理输入特征
        # 1. 如果没有额外特征但模型期望的d_in > 3，只使用xyz
        # 2. 如果有额外特征，需要确保(xyz+features)的维度与d_in匹配
        if features is None:
            if self.d_in <= 3:
                # 只使用坐标的前d_in个维度
                input_features = xyz[:, :, :self.d_in]
            else:
                # 打印警告，使用零填充扩展到d_in维度
                print(f"Warning: Model expects {self.d_in} input features but only xyz(3) provided. Zero-padding will be applied.")
            ...
            if feat_dim == self.d_in:
                ...
                # 如果拼接后的维度过大，截断后面的特征
                print(f"Warning: Combined features dim ({feat_dim}) exceeds model's expected input dim ({self.d_in}). Truncating.")
                input_features = combined_features[:, :, :self.d_in]
            else:
                # 如果拼接后的维度过小，用零填充
                print(f"Warning: Combined features dim ({feat_dim}) is less than model's expected input dim ({self.d_in}). Zero-padding.")
        ...
        # 初始特征转换
        x = self.fc_start(input_features)  # [B, N, 8]
        ...
        # 考虑将输出转换为 [B, N, num_classes] 格式，以便与其他模型保持一致
        # point_scores = point_scores.transpose(1, 2)  # [B, N, num_classes]
