...
class StructureOrientedLoss(nn.Module):
    def __init__(self, num_classes=5, alpha=200.0):
        super().__init__()
    ...
    def forward(self, outputs, labels, points):
        """
        outputs: 模型预测结果 [B, N, C]
        labels: 真实标签 [B, N]
        points: 点云坐标 [B, N, 3] (xyz)
        """
        B, N = labels.shape
        ...
        pred_labels = torch.argmax(pred_probs, dim=-1)  # [B, N]

        # 初始化权重矩阵
        base_weights = torch.ones(B, self.num_classes, device=device)

        # 提取各部件坐标（向量化操作）
        raw_mask = {
            cls: (labels == idx).float().unsqueeze(-1)
            for cls, idx in self.class_map.items()
        }
        pred_mask = {
            cls: (pred_labels == idx).float().unsqueeze(-1)
            for cls, idx in self.class_map.items()
        }
        ...
        for cls in self.class_map:
            raw_points_cls = raw_mask[cls] * points
            pred_points_cls = pred_mask[cls] * points

            raw_z[cls] = (raw_points_cls[:, :, 2].sum(dim=1) /
                          (raw_mask[cls].sum(dim=1) + 1e-7))  # [B]
            pred_z[cls] = (pred_points_cls[:, :, 2].sum(dim=1) /
                           (pred_mask[cls].sum(dim=1) + 1e-7))  # [B]

        # 第一层级关系约束（向量化实现）
        # Girder应在Deck下方
        mask = (raw_z["girder"] < raw_z["deck"]) & (pred_z["girder"] >= pred_z["deck"])
        base_weights[:, self.class_map["girder"]] += self.alpha * mask.float()
        base_weights[:, self.class_map["deck"]] += self.alpha * mask.float()

        # 第二层级关系约束
        # Abutment应在Girder下方
        mask = (raw_z["abutment"] < raw_z["girder"]) & (pred_z["abutment"] >= pred_z["girder"])
        base_weights[:, self.class_map["abutment"]] += 2 * self.alpha * mask.float()
        base_weights[:, self.class_map["girder"]] += 2 * self.alpha * mask.float()

        # 动态权重交叉熵损失
        weights = base_weights / base_weights.sum(dim=1, keepdim=True)  # 归一化
        loss = F.cross_entropy(
            outputs.view(-1, self.num_classes),
            labels.view(-1),
            weight=weights.mean(dim=0)  # 取batch平均作为全局权重
        )
