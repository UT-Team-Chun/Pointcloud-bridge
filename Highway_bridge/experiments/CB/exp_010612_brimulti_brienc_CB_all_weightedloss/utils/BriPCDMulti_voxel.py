...
class BriPCDMulti(Dataset):
    def __init__(self, data_dir, file_list=None, num_points=4096, transform=False,
                 block_size=1.0, sample_rate=0.5, voxel_size=0.03, logger=None):
        """
        添加voxel_size参数控制下采样密度
        Args:
    ...
    def _load_las_file(self, filename):
        ...
        try:
            ...
            colors = np.vstack((las.red, las.green, las.blue)).transpose() / 65535.0 if hasattr(las,
                                                                                                'red') else np.ones_like(
                points)
            labels = np.array(las.classification) if hasattr(las, 'classification') else np.zeros(len(points))

            # 添加体素下采样
            #points, colors, labels = self.voxel_downsample(points, colors, labels)
    ...
    def voxel_downsample(self, points, colors, labels):
        ...
        try:
            ...
            assert len(points) == len(colors) == len(labels), \
                f"Input arrays must have same length: points={len(points)}, colors={len(colors)}, labels={len(labels)}"
            ...
            for i in range(len(points)):
                ...
                voxel_dict[voxel_key]['labels'].append(labels[i])
            ...
            for voxel in voxel_dict.values():
                if voxel['count'] > 0:
                    ...
                    # 使用众数作为体素的标签
                    label = np.bincount(voxel['labels']).argmax()
            ...
            self.logger.info(f"Downsampled from {len(points)} to {len(downsampled_points)} points")

            return downsampled_points, downsampled_colors, downsampled_labels

        except Exception as e:
            self.logger.error(f"Error in voxel_downsample: {str(e)}")
            raise
