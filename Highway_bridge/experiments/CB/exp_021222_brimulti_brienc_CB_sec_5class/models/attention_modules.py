...
class BridgeStructureEncoding(nn.Module):
    ...
    def forward(self, xyz):
        ...
        encoded = self.structure_mlp(combined_features) # [B, C, N, k]

        # # 通道注意力（保持空间维度）
        # attention = self.channel_attention(encoded)  # [B, C, N, k]
        # # 加权融合（原始特征 + 注意力增强特征）
        # encoded = encoded * attention + encoded  # [B, C, N, k]
        #
...
class ColorFeatureExtraction(nn.Module):
    ...
    def forward(self, colors, xyz):
        ...
        return color_features  # [B, 32, N]



class CompositeFeatureFusion(nn.Module):
    def __init__(self, spatial_channels, color_channels, out_channels):
        super().__init__()
    ...
    def forward(self, spatial_features, color_features):
        ...
        # 通道注意力（保持空间维度）
        attention = self.channel_attention(fused_features)  # [B, C, N, k]
        # 加权融合（原始特征 + 注意力增强特征）
        encoded = fused_features * attention + fused_features  # [B, C, N, k]
