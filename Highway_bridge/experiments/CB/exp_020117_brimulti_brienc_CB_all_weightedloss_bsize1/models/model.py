...
class BridgeStructureLoss(nn.Module):
    def __init__(self, num_classes=5, base_alpha=15.0, strict_alpha=30.0, density_threshold=0.1,class_weights_raw=None):
        super().__init__()
        self.base_alpha = base_alpha  # 基础惩罚系数
        self.strict_alpha = strict_alpha  # 严格模式惩罚系数
        self.density_threshold = density_threshold  # 部件密度阈值
        self.class_weights_raw = class_weights

        # 层级定义（基于局部窗口特性）
        self.hierarchy = {
            1: {'strict_below': [2, 3], 'soft_below': [4]},  # abutment
            2: {'strict_above': [1], 'strict_below': [3]},  # girder
        ...
        # 初始化类别权重（根据最新训练结果调整）
        self.register_buffer('class_weights', self.class_weights_raw)
    ...
    def forward(self, outputs, labels, points):
            ...
            for relation in ['strict_above', 'strict_below']:
                ...
                for other_cid in self.hierarchy[cid][relation]:
                    ...
                    if relation == 'strict_above':
                        diff = (ref_z_mean[joint_valid] - z_mean[joint_valid])  # 应保持正差
                        violation = F.relu(-diff + 0.05)  # 允许5%容差
                    else:
        ...
        # ============ 动态权重归一化 ============
        weights = torch.clamp(weights, min=0.5, max=5.0)  # 防止梯度爆炸
