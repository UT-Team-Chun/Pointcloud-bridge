...
class BridgeStructureLoss(nn.Module):
    def __init__(self, num_classes=5, alpha=100.0, span_ratio=1.8, class_weights=None):
        super().__init__()
        self.alpha = alpha
        ...
        # 桥梁部件层级定义（基于结构工程知识）
        self.hierarchy = {
            1: {'name': 'abutment', 'below': [2, 3, 4], 'xy_group': 'foundation'},
            2: {'name': 'girder', 'above': [1], 'below': [3, 4], 'xy_group': 'main_span'},
            3: {'name': 'deck', 'above': [1, 2], 'below': [4], 'xy_group': 'main_span'},
            4: {'name': 'parapet', 'above': [1, 2, 3], 'xy_group': 'boundary'},
            0: {'name': 'other'}
        }

    def _compute_bounds(self, points, mask):
        """计算三维边界框"""
        masked = points * mask.unsqueeze(-1)
        min_vals = masked.min(dim=1)[0]
        max_vals = masked.max(dim=1)[0]
        return torch.stack([min_vals, max_vals], dim=1)  # [B,2,3]
    ...
    def forward(self, outputs, labels, points):
        ...
        for cid in [0, 1, 2, 3, 4]:
            mask = (labels if cid != 0 else preds) == cid  # GT存在性检查
            z_sum = (points[..., 2] * mask).sum(dim=1)
            count = mask.sum(dim=1).clamp(min=1e-7)
            z_centers[cid] = z_sum / count  # [B]

        # 层级约束检查
        for cid in [1, 2, 3, 4]:
            info = self.hierarchy[cid]

            # 上方约束（必须高于指定部件）
            if 'above' in info:
                for lower_cid in info['above']:
                    violation = (z_centers[cid] < z_centers[lower_cid])  # [B]
                    weights[:, cid] += self.alpha * violation.float()
                    weights[:, lower_cid] += self.alpha * violation.float() * 0.5

            # 下方约束（必须低于指定部件）
            if 'below' in info:
                for upper_cid in info['below']:
                    violation = (z_centers[cid] > z_centers[upper_cid])  # [B]
                    weights[:, cid] += self.alpha * violation.float()
                    weights[:, upper_cid] += self.alpha * violation.float() * 0.5

        # ================== 二维平面约束 ==================
        # 计算各部件XY边界
        bounds = {}
        group_bounds = {'foundation': [], 'main_span': [], 'boundary': []}
        for cid in [1, 2, 3, 4]:
            mask = labels == cid
            bounds[cid] = self._compute_bounds(points, mask)  # [B,2,3]

            # 按分组收集边界信息
            if group := self.hierarchy[cid].get('xy_group'):
                group_bounds[group].append(bounds[cid])

        # 同组部件范围一致性检查（如主跨部件应范围相近）
        for group, b_list in group_bounds.items():
            if len(b_list) < 2:
                continue

            # 计算组内最大公共范围
            group_min = torch.stack([b[:, 0, :2] for b in b_list]).min(dim=0)[0]  # [B,2]
            group_max = torch.stack([b[:, 1, :2] for b in b_list]).max(dim=0)[0]  # [B,2]

            # 检查每个部件的偏离程度
            for cid, b in zip([1, 2, 3, 4], b_list):
                if self.hierarchy[cid].get('xy_group') != group:
                    continue

                # 计算部件范围与组范围的比值
                span_ratio = (b[:, 1, :2] - b[:, 0, :2]) / (group_max - group_min + 1e-7)  # [B,2]
                violation = (span_ratio < 1 / self.span_ratio) | (span_ratio > self.span_ratio)
                weights[:, cid] += self.alpha * violation.float().mean(dim=1)

        # ================== 特殊约束 ==================
        # Parapet必须位于最外层（XY边界最大）
        parapet_b = bounds[4][..., :2]  # [B,2,2]
        for cid in [1, 2, 3]:
            other_b = bounds[cid][..., :2]
            # 检查是否有部件超出parapet范围
            x_violation = (other_b[:, 0, 0] < parapet_b[:, 0, 0]) | (other_b[:, 1, 0] > parapet_b[:, 1, 0])
            y_violation = (other_b[:, 0, 1] < parapet_b[:, 0, 1]) | (other_b[:, 1, 1] > parapet_b[:, 1, 1])
            violation = x_violation | y_violation
            weights[:, cid] += self.alpha * violation.float()
            weights[:, 4] += self.alpha * violation.float() * 0.5

        # ================== Other类处理 ==================
        other_mask = (labels == 0).float()
        weights[:, 0] += self.alpha * (1 - other_mask.mean(dim=1)) * 0.2
