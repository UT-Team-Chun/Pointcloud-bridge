...
class ConsistentPositionEncoding(nn.Module):
    def __init__(self, channels=32, freq_bands=6, grid_size=1.0):
        super().__init__()
        self.channels = channels
        self.freq_bands = freq_bands
        ...
        # 生成频率（使用更合理的频率分布）
        # 使用对数空间的频率，可以更好地捕捉不同尺度的空间信息
        freqs = torch.exp(torch.linspace(0., torch.log(torch.tensor(2.0)) * (freq_bands - 1), freq_bands))
        self.register_buffer('freqs', freqs)

        # 计算特征维度
        self.pos_enc_dim = 6 * freq_bands  # xyz各2个编码（sin和cos）* freq_bands

        # 特征变换的MLP
        self.mlp = nn.Sequential(
            nn.Linear(self.pos_enc_dim, channels),
            nn.LayerNorm(channels),
            nn.ReLU(),
            nn.Linear(channels, channels)
        )

    def compute_position_encoding(self, xyz):
        """
        计算稳定的位置编码
        输入:
            xyz: [B, N, 3] 点云坐标
        输出:
            pos_enc: [B, N, channels] 位置编码
        """
        B, N, _ = xyz.shape
        ...
        # 4. 通过MLP转换到目标维度
        encoded = self.mlp(pos_enc)  # [B, N, channels]
        encoded = encoded.permute(0, 2, 1) # [B, channels, N]

        return encoded

    def forward(self, xyz):
        return self.compute_position_encoding(xyz)
