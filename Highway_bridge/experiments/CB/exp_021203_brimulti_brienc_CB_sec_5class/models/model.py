...
class EnhancedPointNet2(nn.Module):
    def __init__(self, num_classes=8):
        ...
        posi_ch=32
        # 位置编码模块
        #self.pos_encoding = EnhancedPositionalEncoding(posi_ch,4,64,)
        self.bri_enc = BridgeStructureEncoding(posi_ch, 32, 4)

        # 颜色特征处理模块
        self.color_encoder = ColorFeatureExtraction(3, 16)
        self.feature_fusion = CompositeFeatureFusion(posi_ch, 16, 29)

        in_chanel2net = 29 + 3 # 3(xyz) + 3(RGB)

        # Encoder
        # 1st layer: input = 3(xyz) + 3(RGB) + 64(pos_encoding) = 70
        self.sa1 = MultiScaleSetAbstraction(1024, [0.1, 0.2],[16, 32], in_chanel2net, [64, 64, 128])
        ...
        self.final_fusion = nn.Sequential(
            nn.Conv1d(768, 256, 1), #128+256+256
            nn.BatchNorm1d(256),
            nn.Conv1d(256, 64, 1),
            nn.BatchNorm1d(64),
            nn.ReLU(),
