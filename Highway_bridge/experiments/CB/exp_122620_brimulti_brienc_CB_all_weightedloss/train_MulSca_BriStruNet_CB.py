...
from models import EnhancedPointNet2
...
def train():
    ...
    #ce_loss = WeightedCrossEntropyLoss(weight=class_weights.to(device))
...
class DiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # 将target转换为one-hot编码 [B, C, N]
        target_one_hot = F.one_hot(target, num_classes=pred.shape[1]).permute(0, 2, 1).float()

        # 计算每个类别的Dice系数
        numerator = 2 * (pred * target_one_hot).sum(dim=-1)
        denominator = pred.sum(dim=-1) + target_one_hot.sum(dim=-1)
