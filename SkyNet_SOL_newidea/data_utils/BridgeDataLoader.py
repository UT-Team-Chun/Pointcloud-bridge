...
def read_las_file(las_path):
    ...
    if os.path.isdir(las_path):
        raise IsADirectoryError(f"{las_path} is a directory, not a file")
    ...
    if hasattr(las, 'classification'):
        ...
        new_labels[mask_0] = 4
        # 将1-4的标签减1
        mask_others = (labels > 0) & (labels <= 4)
        new_labels[mask_others] = labels[mask_others] - 1
        ...
        #     4: 3  # parapet -> 3
        # }
        # labels = np.array([class_mapping[l] for l in labels])


    else:
        # 如果没有标签，设置默认值0
        new_labels = np.zeros_like(x)

    # 组合所有数据
    points = np.column_stack((x, y, z, r, g, b))
    bridge_data = np.column_stack((points, new_labels))
...
class LWBridgeDataset(Dataset):
    def __init__(self, split='train', data_root='trainval_fullarea', num_point=4096, block_size=1.0, sample_rate=1.0,
                 num_class=4, transform=None):
        super().__init__()
        ...
        # 只获取.las文件
        bridges = [f for f in sorted(os.listdir(data_folder)) if
                   f.endswith('.las') and os.path.isfile(os.path.join(data_folder, f))]
        ...
        for bridge in bridges:
            ...
            try:
                ...
                self.bridge_points.append(points)
                self.bridge_labels.append(labels)
                self.bridge_coord_min.append(coord_min)
                self.bridge_coord_max.append(coord_max)
                num_point_all.append(labels.size)
            except Exception as e:
                print(f"Error processing file {bridge}: {e}")
                continue

        if not self.bridge_points:
            raise ValueError(f"No valid .las files were processed in {data_folder}")
        ...
        sample_prob = num_point_all / np.sum(
            num_point_all)  # point_number of each bridge / the total point_number, (probability)
        num_iter = int(np.sum(
            num_point_all) * sample_rate / num_point)  # number of sub point clouds (num_iter times to cover all points)
        bridge_idxs = []
    ...
    def __getitem__(self, idx):
        ...
        while (True):
            center = points[np.random.choice(N_points)][
                     : 3]  # randomly select a point as the center (only use coordinates)
            # draw a rectangle
        ...
        current_points[:, 6] = selected_points[:, 0] / (
                    self.bridge_coord_max[bridge_idx][0] - self.bridge_coord_min[bridge_idx][0])
        current_points[:, 7] = selected_points[:, 1] / (
                    self.bridge_coord_max[bridge_idx][1] - self.bridge_coord_min[bridge_idx][1])
        current_points[:, 8] = selected_points[:, 2] / (
                    self.bridge_coord_max[bridge_idx][2] - self.bridge_coord_min[bridge_idx][2])
...
class ScannetDatasetWholeScene():
    ...
    def __getitem__(self, index):
        ...
        # like convolution
        grid_x = int(
            np.ceil(float(coord_max[0] - coord_min[0] - self.block_size) / self.stride) + 1)  # 'ceiling' and 'floor'
        ...
        for index_y in range(0, grid_y):
            for index_x in range(0, grid_x):
                ...
                point_idxs = np.concatenate(
                    (point_idxs, point_idxs_repeat))  # make 'point_idxs.size' the integer multiple of 4096
...
if __name__ == '__main__':
    ...
    point_data = LWBridgeDataset(split='train', data_root=root_total, num_point=num_point, block_size=block_size,
